<script>
  ...
  const confirmCalculation = () => {
    ...
    setTimeout(() => {
      ...
      missionState.update(s => ({
        ...s,
        studentCalculations: { ...s.studentCalculations, isValid: true, tliDeltaV: val }
      }));
      isConfirmed = true;
      isValidating = false;
    }, 1000);
  ...
  const startSimulation = () => {
    missionState.update(s => ({ ...s, isAnalyzing: false, showBriefing: false, status: "Esecuzione Manovra TLI" }));
  };
      ...
      <div class="bg-nasa-blue/20 border border-nasa-blue/40 p-8 rounded-3xl max-w-lg shadow-2xl animate-in fade-in zoom-in-95">
        ...
        <ul class="space-y-4 text-sm text-gray-300 leading-relaxed italic">
          <li class="flex gap-3">
            ...
            <p>Per raggiungere la Luna, il tuo nuovo <span class="text-white font-bold">apogeo (ra)</span> deve coincidere con la distanza Terra-Luna.</p>
          </li>
          <li class="flex gap-3">
            <span class="text-nasa-blue font-bold">2.</span>
            <div class="flex-1">
              <p>Usa l'equazione di Vis-Viva per trovare la velocità necessaria al perigeo <span class="text-white font-bold">(vp)</span> di questa ellisse.</p>
              <div class="mt-3 bg-black/40 p-2 rounded-xl text-center border border-nasa-blue/20 overflow-x-auto text-[14px]">
                <div use:math={{expression: 'v = \\sqrt{GM \\left( \\frac{2}{r} - \\frac{1}{a} \\right)}', displayMode: true}}></div>
              </div>
            </div>
          </li>
          <li class="flex gap-3">
            <span class="text-nasa-blue font-bold">3.</span>
            <p>Il <span class="text-white font-bold">Δv</span> è la differenza tra questa velocità e quella che hai attualmente in orbita circolare.</p>
          </li>
        </ul>
        <button 
    ...
    <div class="space-y-4 font-mono text-[11px]">
      ...
      {#if teacherMode}
        <div class="mt-8 p-4 bg-yellow-500/10 border border-yellow-500/40 rounded-xl animate-pulse">
          ...
          <p class="text-white text-[10px]">Δv Target: <span class="text-yellow-400">~3134 m/s</span></p>
          <p class="text-gray-400 text-[9px] mt-1 italic">V_circ = 7793 m/s | V_tli = 10927 m/s</p>
        </div>
      {/if}
    </div>
  </div>
  ...
  <div class="flex-1 p-8 flex flex-col justify-between">
    <div>
      <div class="flex justify-between items-center mb-8">
        <h2 class="text-xl font-black uppercase tracking-tighter italic text-white/90">Terminale Calcolo TLI</h2>
      ...
      <div class="space-y-6">
        ...
        {#if isConfirmed}
          <div class="p-4 rounded-xl border border-green-500/30 bg-green-500/10 text-green-400 animate-in fade-in slide-in-from-top-2 flex items-center gap-3">
            ...
            <p class="text-xs font-mono uppercase tracking-widest">Traiettoria Validata. Sistemi pronti.</p>
          </div>
        {/if}
      </div>
    </div>
