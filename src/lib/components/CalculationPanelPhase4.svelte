<script>
  ...
  const confirmCalculation = () => {
    ...
    setTimeout(() => {
      ...
      missionState.update(s => ({
        ...s,
        studentCalculations: { ...s.studentCalculations, isValid: true, loiDeltaV: val }
      }));
      isConfirmed = true;
      isValidating = false;
    }, 1000);
  ...
  const startSimulation = () => {
    missionState.update(s => ({ ...s, isAnalyzing: false, showBriefing: false, status: "Esecuzione Manovra LOI" }));
  };
...
<div class="w-full max-w-4xl bg-black/90 backdrop-blur-2xl border border-nasa-blue/40 rounded-3xl overflow-hidden shadow-2xl flex h-[600px] text-white animate-in zoom-in-95 duration-300 relative">
      ...
      <div class="bg-nasa-blue/20 border border-nasa-blue/40 p-8 rounded-3xl max-w-lg shadow-2xl animate-in fade-in zoom-in-95">
        ...
        <ul class="space-y-4 text-sm text-gray-300 leading-relaxed italic">
          <li class="flex gap-3">
            ...
            <p>Siamo in avvicinamento alla Luna. La quota target del nostro parcheggio è <span class="text-white font-bold">100 km</span> dalla superficie.</p>
          </li>
          <li class="flex gap-3">
            <span class="text-nasa-blue font-bold">2.</span>
            <div class="flex-1">
              <p>Calcola la velocità dell'orbita circolare <span class="text-white font-bold">(v_circ)</span> a quell'altitudine (r = R_Luna + h).</p>
              <div class="mt-3 bg-black/40 p-2 rounded-xl text-center border border-nasa-blue/20 overflow-x-auto text-[14px]">
                <div use:math={{expression: 'v_{circ} = \\sqrt{\\frac{GM_{Luna}}{r}}', displayMode: true}}></div>
              </div>
            </div>
          </li>
          <li class="flex gap-3">
            <span class="text-nasa-blue font-bold">3.</span>
            <p>Il <span class="text-white font-bold">Δv</span> sarà la differenza tra la tua attuale velocità d'arrivo (V_iperbolica) e la velocità calcolata (v_circ). Poiché stiamo frenando, questo valore rappresenta una decelerazione.</p>
          </li>
        </ul>
        <button 
  ...
  <div class="w-1/3 bg-nasa-blue/5 border-r border-white/10 p-8">
    ...
    <div class="space-y-4 font-mono text-[11px]">
      ...
      <div class="p-3 bg-white/5 rounded-lg border border-white/5">
        <p class="text-gray-500 mb-1">Quota Target (h)</p>
      ...
      <div class="p-3 bg-white/5 rounded-lg border border-white/5">
        <p class="text-gray-500 mb-1">Velocità Arrivo (v_arr)</p>
        <p class="text-white">2.515 m/s</p>
      </div>
      ...
      {#if teacherMode}
        <div class="mt-8 p-4 bg-yellow-500/10 border border-yellow-500/40 rounded-xl animate-pulse">
          ...
          <p class="text-white text-[10px]">Δv Target (frenata): <span class="text-yellow-400">~880 m/s</span></p>
          <p class="text-gray-400 text-[9px] mt-1 italic">V_arr = 2515 m/s | V_circ = 1634 m/s</p>
        </div>
      {/if}
    </div>
  </div>
  ...
  <div class="flex-1 p-8 flex flex-col justify-between">
    <div>
      <div class="flex justify-between items-center mb-8">
        <h2 class="text-xl font-black uppercase tracking-tighter italic text-white/90">Terminale Calcolo LOI</h2>
      ...
      <div class="space-y-6">
        <div class="bg-black border border-white/10 p-6 rounded-2xl shadow-inner relative overflow-hidden group">
          ...
          <label for="deltaV" class="block text-[10px] uppercase tracking-widest text-nasa-blue font-bold mb-4 relative z-10">
            Delta-v per inserimento orbita lunare (m/s)
          </label>
          <div class="flex gap-4 relative z-10">
            <input 
              ...
              placeholder="Inserisci Delta-v (es. 880)"
        ...
        {#if isConfirmed}
          <div class="p-4 rounded-xl border border-green-500/30 bg-green-500/10 text-green-400 animate-in fade-in slide-in-from-top-2 flex items-center gap-3">
            ...
            <p class="text-xs font-mono uppercase tracking-widest">Sequenza LOI Validata. Motori pronti.</p>
          </div>
        {/if}
      </div>
    </div>
    ...
    {#if isConfirmed}
      ...
      >
        <span>Esegui Accensione (Retro-Burn)</span>
